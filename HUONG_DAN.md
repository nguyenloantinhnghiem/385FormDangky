...
**Nếu tài khoản của bạn không thấy nút Sync fork:**

Nút **Sync fork** chỉ xuất hiện khi GitHub nhận repo của bạn là một **fork thật**. Nếu bạn tạo repo bằng nút **Deploy with Vercel**, import project, upload code, hoặc tạo repo mới rồi copy code vào, GitHub có thể xem đó là repo độc lập nên không hiện nút này.

Dùng cách đồng bộ bằng **Actions** có sẵn trong dự án:
1. Mở repo của bạn trên GitHub: `github.com/TEN_CUA_BAN/FormDangky`
2. Vào tab **Actions**
3. Nếu GitHub hỏi xác nhận, nhấn **I understand my workflows, go ahead and enable them**
4. Chọn workflow **Đồng bộ từ repo gốc**
5. Nhấn **Run workflow**
6. Giữ `target_branch` là `main`, giữ `upstream_branch` là `main`, rồi nhấn **Run workflow**
7. Chờ workflow chạy xong. Nếu repo đã nối với Vercel, Vercel sẽ tự deploy lại.

Nếu workflow báo lỗi không có quyền push, vào **Settings** → **Actions** → **General** → **Workflow permissions**, chọn **Read and write permissions**, nhấn **Save**, rồi chạy lại workflow.

Nếu tab **Actions** chưa có workflow **Đồng bộ từ repo gốc**, hãy dùng phần Command Prompt bên dưới để sync một lần. Sau lần sync đó, workflow sẽ xuất hiện trong tab **Actions** cho các lần cập nhật tiếp theo.
